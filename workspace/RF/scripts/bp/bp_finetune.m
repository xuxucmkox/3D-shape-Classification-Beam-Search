...
for iter = 1 : param.epochs
    ...
    batch_label = single(zeros(batch_size,2));
...
%     end
    leftright = zeros(model.classes,2);
    ...
    [sorted1,index1] = sort(leftright(:,1),'descend');
    [sorted2,index2] = sort(leftright(:,2),'descend');

    for c = 1:1:model.classes/2
        batch_label(batch_class_label(:,index1(c))==1,1) = 1;
        batch_label(batch_class_label(:,index1(c))==1,2) = 0;
        batch_label(batch_class_label(:,index2(c))==1,1) = 0;
        batch_label(batch_class_label(:,index2(c))==1,2) = 1;
    end
    

    
    gini_impurity = sum(leftright(:,1).*leftright(:,2)/(batch_size/model.classes)^2);
    
    
    [model, loss] = bp_backward(model, activation, batch_label);
    model = bp_update(model, param);
    
    lc = sum(batch_label(:,1))/batch_size*model.classes;
    rc = model.classes - lc;
    fprintf('number of clasees in left: %f,number of clasees in right: %f \n',lc,rc);
